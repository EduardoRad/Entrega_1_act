...
void Magatzem::enmagatzemarComanda(vector<Bicicleta*>& paquet, Data dataEntrada)
{
	...
	m_stock.insert(pair<string, queue<Bicicleta*>>(m_codi, aux));*/
	...
	for (int i = 0;i < models.size(); i++)
	{
		...
		m_stock.insert(pair<string, priority_queue<Bicicleta*>>(models[i], aux));
	}
}
...
bool Magatzem::rebreComanda(Comanda& c, vector<Bicicleta*>& paquet, Data dataRecepcioPaquet)
{
	...
	data = dataRecepcioPaquet;
	for (int i = 0;i < c.getLlistaComanda().size();i++)
	{
		m_relacioModelTipus.insert(pair<TipusBicicleta, vector<string>>(c.getLlistaComanda()[i].tipus, c.getLlistaComanda()[i].posiblesModels));
	}
	return false;
}
...
void Magatzem::generaFactura(bool paquetComplet, float costCorregit, Comanda& c)
{
	...
	if (factura.is_open())
	{
		...
		for (map<string, priority_queue<Bicicleta*>>::iterator it = m_stock.begin();it == m_stock.end(); it++)
		{
